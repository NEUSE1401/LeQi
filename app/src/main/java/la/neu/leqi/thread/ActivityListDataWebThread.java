...
public class ActivityListDataWebThread extends AsyncTask<Void, Void, ArrayList<ActivityBean>> {
    ...
    public ActivityListDataWebThread(String base_url, int type, PullToRefreshBase<ListView> paramPullToRefreshListView, ActivityListViewAdapter paramMyAdapter, Context paramContext) {
        BASE_URL = base_url;
...
//        activityBeen.add(activityBean4);
    ...
    protected void onPostExecute(ArrayList<ActivityBean> activityBeen) {
        ...
        for (ActivityBean c : activityBeen) {
            activityListViewAdapter.add(c);
        }
